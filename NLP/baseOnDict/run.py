        ...
        try:
            ...
            if jsonArray:
                ...
                for one_topic in topics:
                    ...
                    #     topic[4]) + "," + topic[5]
                    topic_score = round(topic[4],2)
                    topic_id = storeTopicToMysql(True, (jobId, topic_title, topic_content, topic[5], source, topic_score,
                                                 topic[6]))
...
def storeTopicToMysql(isOne, data):
    ...
    sqli = "insert into Topic(job_id,title,content,publishTime,source,score,originUrl) values (%s,%s,%s,%s,%s,%s,%s)"
    ...
    try:
        if (isOne):
            ...
            # 连接数据库
            msp = MySolrPy('http://10.21.3.120:8080/solr/yiban_sa')
            bean = {
                'id': id,
                'jobId': data[0],
                'title': data[1],
